...
from modules.video_info import get_video_download_info
from modules.stream_downloader import download_video_stream, create_progress_callback
...
def setup_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument('video_id', help='B站视频BV号或AV号，例如：BV1Ab411C7Q9 或 av123456789')
    parser.add_argument('--output', '-o', default=None, help='输出目录')
    parser.add_argument('--quality', '-q', type=int, default=None, help='指定清晰度（可选）')
    parser.add_argument('--config', '-c', default='config/config.json', help='配置文件路径')
    parser.add_argument('--login', '-l', action='store_true', help='强制重新登录')
    parser.add_argument('--debug', '-d', action='store_true', help='启用调试模式')
    parser.add_argument('--no-merge', action='store_true', help='不自动合并DASH视频')
    return parser
...
def download_video(video_id: str, output_dir: str = None, quality: int = None, no_merge: bool = False) -> bool:
    """主下载函数"""
    try:
        # 解析视频ID
        ...
        # 获取BV号子目录
        from utils.common_utils import get_bvid_directory, ensure_directory
        ...
        from utils.common_utils import truncate_title
        ...
        from utils.common_utils import write_json_file
        ...
        print(f"下载过程异常: {e}")
        import traceback
        if get_config('debug.enable_logging', False):
            traceback.print_exc()
        return False
...
def main():
    ...
    # 处理登录
    if not handle_login(args.login):
        return 1
